...
- 접속: 이 세션 환경은 `add_repo`(Kohgane/kohgane-vault)로 attach 후 clone. LinkLynk 서버 자격은 여기 없음 — 없으면 clone→403 시 "오너 액션: 토큰 Repository access에 kohgane-vault 추가". **토큰 값 로그·출력 금지.**
- **이중 유지 금지**: 소급/핸드오프 상세는 볼트가 정본. CLAUDE.md엔 포인터만(아래 누적 메모리는 히스토리 보존, 신규 상세는 볼트로). 세션 핸드오프 문서 체계는 폐지됨(`docs/session_handoff_v87.md`는 superseded 아카이브).
...
- **[ ] 볼트 노트 갱신(오너 2026-08-18, 완료 5항).** 트랙 종결 시 `kohgane-vault`에 반영 — 새 지뢰 추가·해소된 결정 소거·`프로젝트/고가브릿지` 상태 현행화. 볼트 push는 별 커밋 `[from-cc]` 접두, 오너 push와 충돌 시 **오너 우선·rebase·force 금지**.
- **PR 본문에 셀프 점검 체크리스트 + 캡처 + pytest/CI 결과를 반드시 첨부**한 뒤 머지. "했다"는 문장만으로 닫지 말 것.
- **반쪽 실행 금지:** 항목을 절반만 하고 머지하지 않는다. 절반이면 미완 표시하고 다음으로 안 넘어간다.
- **[ ] 전체 스위트 실행 중 브랜치 전환 금지(오너 2026-08-02).** pytest가 도는 동안 `git checkout`/rebase/stash를
  하면 **실행 중 소스가 바뀌어 결과가 무효**다(그런데 exit code는 0으로 나와 그린으로 오인하기 쉽다).
  다른 브랜치를 만져야 하면 **워크트리를 분리**(`git worktree add`)하거나 **스위트 완주 후** 전환한다.
  이미 전환해 버렸으면 그 실행분은 폐기하고 재실행한다 — 숫자를 그대로 보고하지 않는다.
...
> **이번 스레드 흉터(재발 방지):** ①공허한 그린 부검 3건(v86-H collect-only 위장·W10 워커 고갈·W11 스테일값) → 그린은 필요조건, 실기기·계측 별도 검증 · ②draft 교착 3회 → 셀프 게이트 충족 시 자율 머지 · ③장부 실종(W2-impl 3회) → 보고 첫 블록 트랙 표 강제. 상세=docs/session_handoff_v87.md §1b·§7.
...
## 🟩 아이콘 v8 확정 (오너 2026-07-04 — brand_icons_v2 → 코드 생성 전면 교체)
- ✅ **아이콘 v8 코드 생성 + 전 위치 교체 (#415):** 오너가 확정 지오메트리 스펙 제공 → 이미지 의존 0의 **`scripts/build_icons.py`**
  신설(코드로 렌더, 4x 슈퍼샘플). 스펙(1024 캔버스): 흰 배경 + 먹(#1A1714) 라운드 보더(r190·두께26) / 골드(#C9A24B)
  게이트 아치(중심선 r165·선폭34·중심y430·상단틈 반각0.235rad·다리 x=347/677→y708) / 주황(#F5821F) 키스톤(r29·틈
  정중앙) / 뾰족 타워+2단 받침대(x252/772) / 틸(#119A8E) 데크 2줄(y612/664) / 딥골드(#B28C3A·폭18) 케이블(오목
  사이드스팬 + 타워탑 사이 깊은 처짐 제어점y676). **소형(≤48)=고대비 변형**(build_simple: 얇은 케이블/타워 생략, 굵은
  게이트 아치+큰 주황 키스톤+굵은 틸 데크 — 16px서도 청록·주황 생존, 오너 v39-B 가독 요건 유지). `deploy()`가 전 위치
  교체: static(favicon.ico[16/32/48]·favicon-16/32/48·favicon.svg[512 data-URI 임베드]·apple-touch·icon-180/192/512/1024)
  + 확장 icons(16/32/48 소형변형·128 풀) + assets 마스터. **캐시버스트 ?v=179→180**(단일따옴표 `v='179'` 폼 포함
  16파일), 확장 manifest 1.5.34→**1.5.35**(재로딩 유도). ※오너가 '?v=3' 지시했으나 코드베이스가 이미 v=179라 역행
  방지 위해 단조 증가값 180 사용(캐시 버스트 목적 달성). 옛 다크(#020010 스플래시) 잔재 0·globe 0. 가드
  test_v45_icon_v8(6: 스크립트 API·흰배경+브랜드색·소형 고대비·전 파일 존재·globe/다크 잔재0·캐시버전/manifest bump)
  + 기존 아이콘 가드 15+개 v8 기준 갱신(1.5.35·v=180·흰배경). **검증 캡처:** docs/screens/v45/icon-v8-replace.png
...
- ✅ **북마클릿 토큰 발급 실패 진단 + 회복 (#418):** 증상=/seller/bookmarklet '내 북마클릿 만들기'가 '토큰을 저장하지
  못했어요' 반복. **근본 후보:** append는 성공했는데 **자기검증 읽기(_find_token_row)가 429로 터져** 가짜 실패 → 재시도마다
  토큰만 쌓임. **수리(generate_token):** ①`_classify_token_error`(429/권한401·403/타임아웃5xx/잠금/기타) 1줄 분류 → 서버
  로그에 실제 예외(타입+메시지, 시크릿0)+원인, 사용자 안내에도 원인 포함(정직) ②`_sheet_retry`(429/5xx 지수 백오프 3회)로
  append 감쌈 ③**읽기 실패는 관대 처리**(append 성공이면 durable, '깨끗이 읽었는데 없음'만 진짜 실패) → 가짜 실패·토큰
  중복 발급 박멸. **근본 해결=user_tokens Supabase 이관(#417, 트랜잭션 커밋=durable, 429 없음).** 가드
