...
- ⏳ P0-1 수집버튼: 지정 소싱처에서만(v10/v13 재강조) + **줌(Ctrl+휠)·리사이즈 반응형**(과대·겹침·이탈 방지) + 등록 소싱처 칩+파비콘.
- ⏳ P0-2 마켓연결: "연결하기"가 **새 창/탭 금지 → 같은 페이지 단계 클릭플로우**. 발급URL딥링크+키설명+필수, 쿠팡 출고지 쿠팡한정(v14), 한 마켓씩.
- ⏳ P0-3 확장설치: zip 파일명·표기 **"고가수집기"**(gogasujipgi-vX.Y.Z.zip), `.json`/chrome://extensions/manifest는 '?' 툴팁 뒤로,
  "왜 설치/왜 토큰" 초친절 비유, 페이지당 버튼 1개, "기존 수집기 엔드포인트…" 문구 완전 삭제.
