"""src/forwarding_integration/providers/ohmyzip.py — OhMyZipProvider (Phase 83)."""
...
_WAREHOUSE_US: Dict = {
    'name': '오마이집 미국 창고',
    'address': '1000 Commerce Dr, Los Angeles, CA 90001, USA',
    'country': 'US',
}


class OhMyZipProvider(ForwardingProvider):
    """오마이집(OhMyZip) 배송대행 공급자 Mock 구현."""

    _PROVIDER_ID = 'ohmyzip'
    _NAME = '오마이집 (OhMyZip)'
    ...
    def register_inbound(self, package: ForwardingPackage) -> InboundRegistration:
        ...
        logger.info('OhMyZip 입고 등록: %s pkg=%s', reg.registration_id, package.package_id)
        return reg
    ...
    def confirm_arrival(self, registration_id: str) -> InboundRegistration:
        ...
        reg.status = 'arrived'
        logger.info('OhMyZip 도착 확인: %s', registration_id)
        return reg
    ...
    def track_package(self, package_id: str) -> ForwardingStatus:
        ...
        if status is None:
            status = ForwardingStatus(
                ...
                provider=self._PROVIDER_ID,
                tracking_number=f"OMZ{uuid.uuid4().hex[:12].upper()}",
