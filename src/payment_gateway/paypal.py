"""src/payment_gateway/paypal.py — Phase 45: PayPal 어댑터 (mock)."""
import logging
import uuid
from datetime import datetime, timezone
from typing import Optional

from .gateway_manager import PaymentGateway

logger = logging.getLogger(__name__)


class PayPalGateway(PaymentGateway):
    """PayPal mock 구현."""

    def __init__(self, client_id: str = 'mock_pp_client',
                 client_secret: str = 'mock_pp_secret'):
        self._client_id = client_id
        self._client_secret = client_secret
        self._payments = {}

    def initiate_payment(self, amount: float, currency: str = 'USD',
                         order_id: str = '', **kwargs) -> dict:
        payment_id = f"PAYID-{uuid.uuid4().hex[:20].upper()}"
        payment = {
            'payment_id': payment_id,
            'gateway': 'paypal',
            'amount': amount,
            'currency': currency,
            'order_id': order_id,
            'status': 'created',
            'approval_url': f"https://www.sandbox.paypal.com/checkoutnow?token={payment_id}",
            'created_at': datetime.now(timezone.utc).isoformat(),
        }
        self._payments[payment_id] = payment
        logger.info("[PayPal mock] 결제 생성: %s", payment_id)
        return payment

    def confirm_payment(self, payment_id: str, **kwargs) -> dict:
        payment = self._payments.get(payment_id)
        if payment is None:
            raise KeyError(f"결제 없음: {payment_id}")
        payment['status'] = 'approved'
        payment['confirmed_at'] = datetime.now(timezone.utc).isoformat()
        return payment

    def refund_payment(self, payment_id: str, amount: Optional[float] = None, **kwargs) -> dict:
        payment = self._payments.get(payment_id)
        if payment is None:
            raise KeyError(f"결제 없음: {payment_id}")
        refund_amount = amount if amount is not None else payment['amount']
        payment['status'] = 'refunded'
        payment['refund_amount'] = refund_amount
        payment['refunded_at'] = datetime.now(timezone.utc).isoformat()
        return payment

    def get_status(self, payment_id: str) -> dict:
        payment = self._payments.get(payment_id)
        if payment is None:
            return {'payment_id': payment_id, 'status': 'not_found'}
        return {'payment_id': payment_id, 'status': payment['status']}
