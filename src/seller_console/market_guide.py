...
MARKET_GUIDE: List[Dict[str, Any]] = [
    ...
    {
        # K2 — 연동대행사 모델. 대행사(고가브릿지) 등록 심사가 통과해야 판매자 매핑이 열린다.
        #   발급 경로 URL은 **문서 실측 전이라 적지 않는다**(없는 링크를 만들지 않는다).
        "key": "talkstore",
        "label": "톡스토어 (카카오)",
        "icon": "🟨",
        "status": "planned",
        "flow": ["고가브릿지 대행사 등록(심사 중)", "판매자 API 인증키 발급", "매핑 요청", "여기에 붙여넣기"],
        "steps": [
            {"t": "고가브릿지가 연동대행사로 등록되는 중입니다",
             "d": "다른 마켓과 달리 판매자가 앱을 직접 만들지 않습니다. 저희가 대행사로 승인되면 "
                  "판매자는 자기 인증키만 넣으면 됩니다."},
            {"t": "톡스토어 판매자센터에서 API 인증키를 발급받습니다",
             "d": "발급 화면 경로는 대행사 승인 안내와 함께 확정되면 여기에 채워 드립니다."},
            {"t": "인증키를 아래에 붙여넣고 매핑을 요청합니다",
             "d": "저희 서버가 대행사 앱 키를 갖고 있어 판매자는 Admin키를 넣지 않습니다."},
        ],
        "fields": [
            {"env": "TALKSTORE_SELLER_API_KEY", "label": "판매자 API 인증키",
             "where": "톡스토어 판매자센터에서 발급"},
            {"env": "TALKSTORE_STORE_ID", "label": "스토어 ID", "where": "매핑 대상 스토어 구분용(선택)"},
        ],
        "tips": [
            "지금은 입력만 받아 두고 등록은 열리지 않습니다 — 대행사 승인과 규격 확인이 끝나면 열립니다.",
            "판매자 한 곳이 동시에 매핑할 수 있는 대행사 수에 상한이 있습니다(정확한 수치는 확정 후 안내).",
        ],
    },
    # ── 글로벌 확장 (어댑터 구현 예정) ─────────────────────────────────────────
    {
