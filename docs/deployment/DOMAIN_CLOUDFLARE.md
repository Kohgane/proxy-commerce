...
> **대상**: `kohganepercentiii.com` → Render 서비스 연결  
...
5. `kohganepercentiii.com` 항목에서 **Purchase** 클릭
...
1. Cloudflare 대시보드 → 좌측 **Websites** → `kohganepercentiii.com` 클릭
...
4. `kohganepercentiii.com` 입력 → **Save** 클릭

   > 📸 _[스크린샷: Custom Domains 추가 화면]_

5. 동일하게 `www.kohganepercentiii.com` 도 추가
...
1. [https://dash.cloudflare.com](https://dash.cloudflare.com) → `kohganepercentiii.com` 클릭
2. 상단 **DNS** 탭 클릭 → **DNS Records** 섹션

### 레코드 추가

#### 루트 도메인 (`kohganepercentiii.com`)

| 항목 | 값 |
|------|----|
| **Type** | `CNAME` |
| **Name** | `@` (또는 `kohganepercentiii.com`) |
...
#### www 서브도메인 (`www.kohganepercentiii.com`)
...
1. `kohganepercentiii.com` → **SSL/TLS** 탭 클릭
...
```bash
# DNS 레코드 확인
dig kohganepercentiii.com CNAME
dig www.kohganepercentiii.com CNAME

# HTTPS 응답 확인
curl -I https://kohganepercentiii.com/health
curl -I https://www.kohganepercentiii.com/health
...
python scripts/cloudflare_setup.py --apex kohganepercentiii.com --dry-run
```
...
- **해결**:
  1. `dig kohganepercentiii.com` 으로 DNS가 올바른지 확인
...
- **해결**:
  1. `ping kohganepercentiii.com` 으로 IP 확인
...
python scripts/cloudflare_setup.py \
    --apex kohganepercentiii.com \
    --target kohganemultishop.onrender.com
```

---

## 13. Hobby Tier 도메인 2개 제한 대처법

Render Hobby Tier는 커스텀 도메인을 **최대 2개**만 허용합니다.  
`kohganemultishop.org` + `www.kohganemultishop.org`처럼 기존 슬롯 2개가 가득 찬 상태에서 새 도메인을 추가하면 다음 에러가 발생합니다:

```
HTTP 400 — {"message":"Hobby Tier is limited to 2 custom domains"}
```

### 현재 슬롯 확인

```bash
export RENDER_API_TOKEN=<토큰>
python scripts/render_domain_attach.py \
    --service-id srv-d78d5rfkijhs73868f8g \
    --list-domains
# 출력 예:
#   도메인 슬롯: 2/2 사용 중 (Hobby Tier 기준)
#   ✓  kohganemultishop.org  [verified]
#   ✓  www.kohganemultishop.org  [verified]
```
...
python scripts/render_domain_attach.py \
    --service-id srv-d78d5rfkijhs73868f8g \
    --remove-domain kohganemultishop.org

python scripts/render_domain_attach.py \
    --service-id srv-d78d5rfkijhs73868f8g \
    --remove-domain www.kohganemultishop.org
```

### 또는 Render Pro로 업그레이드
...
업그레이드: Render 대시보드 → 서비스 → **Upgrade Plan**

---

## 14. 올바른 onrender.com 호스트 찾기

> ⚠️ `proxy-commerce-xxxx.onrender.com`의 `xxxx`를 실제 값으로 반드시 바꿔야 합니다.  
> placeholder 그대로 사용하면 Cloudflare CNAME이 가짜 호스트를 가리켜 **403 Forbidden**이 됩니다.

### 방법 1: Render 대시보드에서 확인

1. [https://dashboard.render.com](https://dashboard.render.com) → **proxy-commerce** 클릭
2. 상단 URL 표시 영역에서 `proxy-commerce-xxxx.onrender.com` 확인

> 📸 _[스크린샷: Render 서비스 URL 표시 영역]_

### 방법 2: API로 확인 (한 줄)

```bash
curl -s -H "Authorization: Bearer ${RENDER_API_TOKEN}" \
    https://api.render.com/v1/services/srv-d78d5rfkijhs73868f8g \
    | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('serviceDetails',{}).get('url') or d.get('url',''))"
```

### 방법 3: `--target auto` 사용 (권장)

```bash
export CF_API_TOKEN=<Cloudflare_토큰>
export RENDER_API_TOKEN=<Render_토큰>
...
`--target auto`는 내부적으로 Render API를 호출해 실제 호스트를 자동 조회하므로  
placeholder를 수동으로 바꿀 필요가 없습니다.

---

## 15. DNS 전파 + SSL 발급 검증 흐름

DNS 레코드를 저장한 후 **단계별 체크포인트**를 따라 검증하세요.

### 체크포인트 타임라인

| 시간 | 체크 항목 | 명령어 |
|------|----------|--------|
| 즉시 | Cloudflare DNS 레코드 저장 확인 | Cloudflare 대시보드 → DNS 탭 |
| 5분 후 | Cloudflare DoH 전파 확인 | `dig @1.1.1.1 kohganepercentiii.com CNAME` |
| 10분 후 | Render 도메인 검증 상태 | `python scripts/render_domain_attach.py --list-domains` |
| 15분 후 | HTTPS 접근 시도 | `curl -I https://kohganepercentiii.com/health` |
| 30분 후 | 최종 스모크 테스트 | `python scripts/render_smoke.py https://kohganepercentiii.com` |
...
### SSL 발급 안 되는 경우 트러블슈팅

#### CAA 레코드 확인

```bash
dig kohganepercentiii.com CAA
```

CAA 레코드가 있으면 Let's Encrypt 허용 항목 추가:
...
#### Cloudflare Proxy ON/OFF

| 상태 | 증상 | 해결 |
|------|------|------|
| Proxy **ON** (주황) | 525 SSL Handshake Failed | → DNS only (회색)으로 변경 |
| Proxy **OFF** (회색) | 인증서 발급 지연 | → 정상, 30분 대기 |
| Proxy **ON** + Full (strict) | 526 Invalid SSL | → Full (strict) → Full로 변경 |

---

## 16. 실제 사용자 시나리오 — kohganemultishop.org에서 kohganepercentiii.com으로 이전

**목표**: 다운타임 최소화 (서비스 중단 없이 도메인 이전)

### 권장 순서 (다운타임 최소화)

```
1단계: 새 도메인 먼저 추가 → 검증 완료
2단계: SSL 발급 완료 확인
3단계: 구 도메인 제거 (이 시점에 잠깐 서비스 중단)
```
...
**Step 2: 슬롯이 가득 차 있으면 — 구 도메인 중 하나 제거**

```bash
# kohganemultishop.org와 www 중 하나를 먼저 제거
python scripts/render_domain_attach.py --remove-domain www.kohganemultishop.org
```
...
```bash
python scripts/render_domain_attach.py \
    --domains kohganepercentiii.com
# 슬롯 2/2이면 위에서 제거한 1개 추가 후 나머지도 순서대로 교체
```
...
## 17. 후속 액션 가이드 (PR 머지 후)

1. **(이미 한 사람은 skip)** Render에서 `kohganemultishop.org`, `www.kohganemultishop.org` 제거
...
5. 30분 대기 후 확인:
   ...
   curl -I https://kohganepercentiii.com/health
   ```
6. 사이트 화면 캡처 보내서 다음 UX 개선 PR 큐에 추가
