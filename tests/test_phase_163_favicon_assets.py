...
def test_base_templates_include_favicon_links():
    ...
    for path in targets:
        ...
        assert ("?v=178" in text) or ("v='178'" in text)
...
def test_public_templates_use_cache_busted_favicon_links():
    ...
    for path in targets:
        text = path.read_text(encoding="utf-8")
        assert "favicon.svg?v=178" in text
        assert "favicon.ico?v=178" in text
        assert "apple-touch-icon.png?v=178" in text
        assert "manifest.webmanifest?v=178" in text
