...
class TestCdStagingWorkflow:
    ...
    def test_render_hook_has_empty_check(self):
        ...
        assert 'if [ -z "$RENDER_DEPLOY_HOOK_STAGING" ]' in self.content or \
               "RENDER_DEPLOY_HOOK_STAGING" in self.content

    def test_healthcheck_has_app_url_check(self):
        """Post-deploy healthcheck 스텝에 APP_URL 조건이 있어야 한다."""
        assert 'if [ -z "$STAGING_APP_URL" ]' in self.content or \
               "STAGING_APP_URL" in self.content

    def test_notify_success_has_telegram_check(self):
        """Notify success 스텝에 텔레그램 토큰 조건이 있어야 한다."""
        assert 'if [ -z "$TELEGRAM_BOT_TOKEN" ]' in self.content or \
               "STAGING_TELEGRAM_BOT_TOKEN" in self.content
    ...
    def test_curl_not_called_unconditionally(self):
        """curl 이 조건 없이 직접 호출되지 않아야 한다."""
        lines = self.content.splitlines()
        for i, line in enumerate(lines):
            if "curl -fsSL -X POST" in line and "RENDER_DEPLOY_HOOK" in line:
                preceding = "\n".join(lines[max(0, i - 5):i])
                # 쉘 if 또는 YAML if: 둘 다 허용
                assert "if " in preceding or "if:" in preceding, (
                    f"curl 호출 (줄 {i + 1}) 앞에 조건이 없습니다."
                )
...
class TestCdProductionWorkflow:
    ...
    def test_render_hook_has_empty_check(self):
        """Deploy to Render 스텝에 시크릿 빈값 조건이 있어야 한다."""
        assert 'if [ -z "$RENDER_DEPLOY_HOOK_PRODUCTION" ]' in self.content or \
               "RENDER_DEPLOY_HOOK_PRODUCTION" in self.content

    def test_healthcheck_has_app_url_check(self):
        """Post-deploy healthcheck 스텝에 APP_URL 조건이 있어야 한다."""
        assert 'if [ -z "$PRODUCTION_APP_URL" ]' in self.content or \
               "PRODUCTION_APP_URL" in self.content
