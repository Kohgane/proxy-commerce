...
def test_amazon_search_renders_bulk_bar_and_all_tile_buttons():
    ...
    # v86-C(오너 철회 2026-07-31): STEP4.1의 '상시 노출' 승인이 거둬들여져 rest=0 호버 노출로 회귀했다.
    #   버튼이 **몇 개 붙었는가**는 위 quick==24가 이미 못박는다. 여기서는 '보이는 개수'가 0인지만 본다.
    assert info["persistentQuick"] == 0, ("rest에서 보이는 [수집]이 있다 — 호버 노출 위반", info)
