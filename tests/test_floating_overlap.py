...
실측(수리 전, 1920×940):
- orders `CSV 내보내기` — 1872px². **이게 본체다**: 페이지가 아예 안 스크롤되는 높이라
  더 내려서 꺼낼 수도 없다(탈출구 0).
- reject-watch `처방 실행` — 620~690px². 다만 이건 **스크롤 위치에 따라 달라진다**
  (분류표 아래 카드가 더 있으면 완전 최하단에선 비켜난다). 오너 화면에선 덮여 있었다.
