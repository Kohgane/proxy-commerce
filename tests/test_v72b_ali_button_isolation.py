...
def test_manifest_bumped():
    assert MANIFEST["version"] == "1.5.112"
...
def test_banner_version_compare_node():
    ...
    harness = (
        m.group(0) + "\n"
        "var out={a:cmp('1.5.9','1.5.112'), b:cmp('1.5.112','1.5.112'), c:cmp('1.6.0','1.5.112')};\n"
    ...
    try:
        ...
        Path(f.name).unlink()
    assert out["a"] == -1        # 1.5.9 < 1.5.112(구버전 감지 — 숫자 비교, 문자열 아님)
    assert out["b"] == 0         # 동일 = 최신
    assert out["c"] == 1         # 1.6.0 > 1.5.112
