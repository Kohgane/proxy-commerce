...
def dashboard_web_client(mock_env, monkeypatch):
    """대시보드 웹 UI Blueprint이 등록된 Flask 테스트 클라이언트(관리자 세션).

    v87 S1.5: /dashboard/* 가 관리자 전용 게이트 뒤로 들어가서, 기능 계약은 관리자 세션으로 호출한다.
    비인증 차단 계약은 tests/test_v87_dashboard_auth_gate.py 담당.
    """
    monkeypatch.setenv('DASHBOARD_WEB_UI_ENABLED', '1')
    monkeypatch.setenv('GOOGLE_SHEET_ID', 'test-sheet-id')

    # 공용 order_webhook.app 대신 격리 앱 — 공용 앱에 세션을 심으면 시트 기반 SECRET_KEY/자격증명
    # 초기화가 걸려 뒤에 도는 다른 테스트(test_billing 등)를 오염시킨다.
