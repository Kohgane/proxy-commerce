...
지정한 **소싱처**(타오바오·티몰·1688·테무·아마존·알리익스프레스 등)에서 한 클릭으로 상품을 코고가네에 수집합니다.
...
1. 상품 페이지에 접속하면 **우하단에 고가네 글로브 아이콘 + “고가네 수집” 버튼**이 자동으로 떠 있음
...
3. 화면 상단 **고가네 수집 바**에서 `전체 선택` / `선택 수집` / `전체 수집`
4. 선택한 상품들이 **한 번에** 수집돼 셀러 콘솔 **수집 이력**에 쌓임(번역 포함)

> 목록 페이지 수집은 카드에 보이는 정보(제목·이미지·가격·링크)를 모읍니다.
> 상세설명·옵션·리뷰까지 필요하면 그 상품의 상세 페이지에서 ‘고가네 수집’을 누르세요.
